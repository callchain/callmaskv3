...
function humanAmount(tx, amount) {
    ...
    return amount.currency == 'CALL' ?
        val.toFormat() + ' CALL'
        : val.toFormat() + ' ' + amount.currency
}
...
/**
 * return { success, icon, type, amount, id }
 * 
 * @param {*} tx 
 * @param {*} address 
 * @returns 
 */
export const txInfo = (tx, address) => {
    const success = tx.outcome.result == 'tesSUCCESS'
    const id = tx.id
    const type = tx.type.slice(0,1).toUpperCase() + tx.type.slice(1)

    let icon = 'mdi-help-box'
    let amount = tx.outcome.fee + ' CALL'
    switch (tx.type) {
        case 'payment':
            icon = address == tx.address ? 'mdi-arrow-up-box' : 'mdi-arrow-down-box'
            amount = tx.outcome.deliveredAmount.value + ' ' + tx.outcome.deliveredAmount.currency
            break;
        case 'order':
        ...
        case 'orderCancellation':
            ...
            break;
        case 'trustline':
            icon = address == tx.address ? 'mdi-archive-lock' : 'mdi-archive-open'
            break;
        case 'settings':
            icon = 'mdi-cog'
            break;
